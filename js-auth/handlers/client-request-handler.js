  ...
  return new Promise(async (resolve, reject) =>
  {
    try {
      ...
      if (payload) {
        ...
        const loggedIn = deviceStore.getInstance().getDeviceIdFor(user["_id"]);
  ...
  return new Promise(async (resolve, reject) =>
  {
    try {
      ...
      if (user) {
        ...
        const loggedIn = deviceStore.getInstance().getDeviceIdFor(user["_id"]);
        if (!loggedIn || loggedIn === device["clientID"]) {
          deviceStore.getInstance().registerUserToDevice(device["clientID"], user["_id"]);
          resolve(user);
        } else
          reject({
            message: "User logged in with same account on other device.",
...
async function handleSignOut(_id, ack)
{
  tokenStore
    ...
    .then((result) =>
    {
      deviceStore.getInstance().removeUserFromDevice(_id);
...
async function handleRevokeAccess(id)
{
  return new Promise(async (resolve, reject) =>
  {
    try {
      ...
      deviceStore.getInstance().removeUserFromDevice(id);
...
async function handleAuthHandshake(data, ack)
{
  ...
  const exists = deviceStore.getInstance().getDeviceIdFor(user["_id"]);
...
{
  deviceStore.getInstance().addDevice(deviceID, socketID);
}

function removeDevice(deviceID)
{
  deviceStore.getInstance().removeDevice(deviceID);
}

function registerUserToDevice(deviceID, userID)
{
  deviceStore.getInstance().registerUserToDevice(deviceID, userID);
}

function removeUserFromDevice(userID)
{
  deviceStore.getInstance().removeUserFromDevice(userID);
}
